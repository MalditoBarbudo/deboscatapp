...
+ Same episodes (distances below 5 km)
  ...
  - "05-002": Seems to be the same episode, but with a polygon 3.5 km away
  - "31-005": Seems to be the same episode, but with a polygon 1.5 km away
...
```
  ...
  years in the same place as 04-014. No fix here.
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[11+1]]`: Seriously???? What do I do here???? Nothing, no fix
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[13+1]]`: Same as before, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[13+1]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[14+1]]`: Episodes evolving, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[14+1]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[21+1]]`: 07-026 seems the continuation of 07-019 for 2020,
  fix it
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[23+1]]`: episode faciting another, no fix.
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[26+1]]`: 13-004 ok, no fix here
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[28+1]]`: 14-066 for 2020 is really 14-013, fix it
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[31+1]]`: Inexplicable, no fix
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[44+1]]`: 23-010 and 23-011 are exchanged in 2021. Fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[44+1]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[45+1]]`: 24-001 in 2012 is really 24-032. Fix it.
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[52+1]]`: 24-073-2015 is a duplicated of 24-064, remove it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[52+1]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[53+1]]`: No idea, no fix.
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[69+1]]`: 31-005 in 2022 should be 31-009, fix it
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[72+1]]`: What the fuck??? No fix
...
```
  ...
  1. `r data_muddy_episodes_centroids_intersection[[79+1]]`: Different episodes after years
...
data_muddy_deboscat_centroids <-
  ...
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '23-010' & year %in% c(2021), glue::glue('23-011'), dplyr::if_else(
      episode_id == '23-011' & year %in% c(2021), glue::glue('23-010'), episode_id
    )
    )
  ) %>%
  dplyr::mutate(
