...
Also, as we fix the year mismatches, we will pre clean the raw data:
...
to fix them, so we do it manually for each one:
  ...
  the 70 percent decoloration value.
...
We see that we have 12 episodes that can be new episodes with wrong code (reusing the code). 10 of them
...
+ No fixes cases:  
  - "06-027": In this case, 2013 doesn't intersect with 2012, but it does with following years. Also,
  distance from 2012 and following years is slightly bigger than 1000m (~ 1080m), so I'm gonna assume that
  is the same episode all years, so no fix needed.
  - "06-045": Same case as the previous episode, in this case the distance is ~1200m, so I'm gonna assume that
  is the same episode, no fix needed.
...
```{r}
create_polygon_plot('32-007', data_dirty_2013, data_dirty_2012, data_dirty_2014_2020 %>% filter(year > 2013))
...
data_dirty_2014_new_species_others_fixed <-
  ...
  dplyr::mutate(
    episode_id = dplyr::case_when(
      episode_id == '14-026' ~ '14-062',
      episode_id == '14-062' ~ '14-026',
      TRUE ~ episode_id
    )
  )
```
...
data_dirty_2015_error_fixed <-
  ...
  dplyr::filter(
    !episode_id %in% c("32-007", "32-008", "32-004", "32-005", "32-006")
  )
```
        ...
        res <- episode_data %>%
          dplyr::mutate(
            species_id = dplyr::if_else(
              species_id == 'Acer campestre' & cover_perc == 10, 'Acer monspesulanum', species_id
            )
          )
      }
...
We see that we have 13 episodes that can be new episodes with wrong code (reusing the code). All episodes
don't intersect with old polygons and 11 of them have new species:
...
+ What the fuck cases:
  ...
  sure there is intersection with future polygons because it dissapears until 2020.
...
data_dirty_2016_error_fixed <-
  ...
  dplyr::filter(
    !episode_id %in% c("32-002", "32-013", "32-009")
  ) %>%
  # new episodes must be checked as new
  dplyr::mutate(
    new_episode = dplyr::if_else(
      episode_id %in% c(
        '02-010', '06-048', '24-073', '31-001', '31-002',
        '31-004', '31-006', '31-007', '32-007', '32-012'
      ),
      TRUE,
      new_episode
    )
...
data_clean_2016 <- list(
  data_dirty_2016_ok, data_dirty_2016_duplicated_species_fixed,
...
don't intersect with old polygons and 9 of them have new species:
...
+ Code reuse cases:
  ...
  Code reusing here, so I will change the code in the old episodes.
  - "32-001": **Finally**, it intersects with the future ones.
  - "32-004": **Finally**, it intersects with the future ones.
...
data_dirty_2017_error_fixed <-
  ...
  dplyr::mutate(
    new_episode = dplyr::if_else(
      episode_id %in% c(
        '18-001', '25-037', '31-008', '31-009', '32-001',
        '32-004', '32-008', '32-009', '32-013'
      ),
      TRUE,
      new_episode
    )
...
data_clean_2017 <- list(
  data_dirty_2017_ok, data_dirty_2017_duplicated_species_fixed, data_dirty_2017_duplicated_species_wrong_poly_fixed,
...
We repeat the process for 2018 data:
...
disguise. We have 3 episodes with this species as new:
  ...
  - "04-033", "24-034" and "35-009" are all mistakes, they should be *Pinus nigra*
...
data_dirty_2018_new_species_populus_fixed <-
  ...
  purrr::imap_dfr(
    function(episode_data, episode_name) {
      if (episode_name %in% c("04-033", "24-034", "35-009")) {
        res <- episode_data %>%
          dplyr::mutate(species_id = if_else(species_id == 'Populus nigra', 'Pinus nigra', species_id))
      } else {
        res <- episode_data
...
In the "20-014" episode, there is no polygon, only data, so this episode has an area of zero. We need to
convert it to NA to avoid interpretation of zero area.

```{r extra_2018}
data_dirty_2018_ok <- data_dirty_2018_ok %>%
  dplyr::mutate(
    episode_area = if_else(episode_id == "20-014", NA_real_, episode_area)
  )
```
...
data_dirty_2019_error <- data_dirty_2019_checks %>%
  dplyr::filter(check_meaning == 'New or error')
```
...
We see that we have 1 episode that can be a new episode with wrong code (reusing the code). The episode
doesn't intersect with old polygons and has new species:

+ No fix:
  
  - "06-018": Slightly above 1 km of distance, it has a new species, but makes sense.
  
```{r no_fix_plots_2019}
create_polygon_plot('06-018', data_dirty_2019, data_dirty_deboscat, data_dirty_2014_2020 %>% filter(year > 2019))
```
  
So nothing to do

```{r error_or_new_2019_fix}
data_dirty_2019_error_fixed <-
  data_dirty_2019_error
```
...
data_dirty_2020_error <- data_dirty_2020_checks %>%
  dplyr::filter(check_meaning == 'New or error')
```
...
+ Code reuse:
  ...
  - "32-002": New species, more than 6 km away, it seems like a new episode
  
```{r code_reuse_plots_2020}
create_polygon_plot('32-002', data_dirty_2020, data_dirty_deboscat, data_dirty_2014_2020 %>% filter(year > 2020))
```
  
```{r error_or_new_2020_fix}
data_dirty_2020_error_fixed <-
  data_dirty_2020_error %>%
  # new episodes must be checked as new
  dplyr::mutate(
    new_episode = dplyr::if_else(
      episode_id %in% c(
...
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[1]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[2]]`: 04-036 for 2017 (which was deemed as no fix for the
  new or error category) seems to be exactly the same as 04-004. I will remove this year form 04-036.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[2]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[3]]`: Similar as before. But in this case is the 04-005 for
  2017 that seems to represent 04-036 in 2017. But not sure here. Very strange. So I will no change anything
  here.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[3]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[4]]`: Here is strange. It seems like 04-031 from 2016 to
  2018 is the continuation of 04-026. I will change this.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[4]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[5]]`: 04-038 seems to be the same as 04-033. It only exist
  one year (2016) and is the same as 04-033. I will remove this one.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[5]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[6]]`: 04-040, new episode after several years in the same
  place as 04-014. No fix here.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[6]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[7]]`: 06-006 and 06-007. They are the same episode, but
...
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[7]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[8]]`: 06-008 and 06-015. I don't understand why they
  are different episodes, but I'm not gonna fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[8]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[9]]`: 06-010 and 06-018. 06-018 is crazy, it is dancing
  around 06-010. Why they are different episodes is above my pay grade. No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[9]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[10]]`: Seriously???? What do I do here???? Nothing, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[10]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[11]]`: Same, all 06-*** are fucked up
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[11]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[12]]`: 06-018 for 2019 is really 06-043
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[12]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[13]]`: 06-027 and 06-045 are changed one for another in
  2012. Fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[13]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[14]]`: they seem changed for 2012, *BUT*,
  06-048-2015 really seems like 06-027 from before :( I don't have a fucking clue what is
  hapenning with 06, I'm starting to think to remove them all.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[14]], data_muddy_deboscat_checks)
```

  > I decided to remove all 06 episodes. They are crazy. I don't have the time or the will to start
  playing detectives to see when the **don't** fuck up (it would be easier than look where they ficked up).
  This would be avoidable if a follow up with the data were done every year.

  1. `r data_muddy_episodes_centroids_intersection[[17]]`: 07-026 seems the continuation of 07-019 for 2020,
  fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[17]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[18]]`: 10-004 polygon for 2015 is wrong, but I can not
  fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[18]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[19]]`: 10-040 is exactly the same as 19-040. Remove it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[19]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[20]]`: Exactly the same as before.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[20]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[21]]`: New episode years later in the same place, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[21]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[22]]`: 14-066 for 2020 is really 14-013, fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[22]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[23]]`: What??, and see `r data_muddy_episodes_centroids_intersection[[26]]`
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[23]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[24]]`: Really??, 2 episodes?? No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[24]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[25]]`: Inexplicable, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[25]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[26]]`: Really?? and see `r data_muddy_episodes_centroids_intersection[[23]]`. It seems like the 2017 is a mix of the three, who knows.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[26]], data_muddy_deboscat_checks)
create_polygon_plot_2(
  c(data_muddy_episodes_centroids_intersection[[23]], data_muddy_episodes_centroids_intersection[[26]]),
  data_muddy_deboscat_checks
)
```

  1. `r data_muddy_episodes_centroids_intersection[[27]]`: 14-078 dance around 14-075 along the years, yujuu!.
  No fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[27]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[28]]`: 16-002 for 2014 is a copy of 16-003, remove it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[28]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[29]]`: 19-002 is broken in two parts in 2016, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[29]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[30]]`: 20-012-2012 is really 20-005, fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[30]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[31]]`: Let's broke the episodes in pieces!! No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[31]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[32]]`: 21-031 in 2018 (only year available for this episode)
  is in reality 24-031. Fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[32]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[33]]`: New episode after years, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[33]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[34]]`: 38-002 in 2012 is a duplicated of 23-002. Also,
  38-002 in 2015 is wring and is again 23-002. Fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[34]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[35]]`: 24-001 in 2012 is really 24-032. Fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[35]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[36]]`: 24-016 in 2019 is really 24-106. Fix it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[36]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[37]]`: Two different episodes, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[37]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[38]]`: 2Two different episodes, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[38]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[39]]`: 24-043 in 2012 is a duplicated of 24-044, remove
  it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[39]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[40]]`: Not sure what happened here, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[40]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[41]]`: Not sure what happened here, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[41]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[42]]`: 24-073-2015 is a duplicated of 24-064, remove it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[42]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[43]]`: No idea, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[43]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[44]]`: Different episodes after years, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[44]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[45]]`: Different episodes, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[45]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[46]]`: Different episodes, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[46]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[47]]`: Different episodes, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[47]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[48]]`: 25-037-2016 is the same as 25-029, change it.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[48]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[49]]`: Different episodes, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[49]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[50]]`: Different episodes, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[50]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[51]]`: 27-001 in 2017 is in reality 28-001. Fix it
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[51]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[52]]`: No idea, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[52]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[53]]`: 31-001-2012 is in reality 31-007-2015
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[53]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[54]]`: 31-007-2012 is in reality 31-001-2015
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[54]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[55]]`: 31-002-2012 is in reality 31-008-2016
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[55]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[56]]`: 31-008-2012 is in reality 31-002-2015
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[56]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[57]]`: No idea no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[57]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[58]]`: No idea no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[58]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[59]]`: No idea, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[59]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[60]]`: What the fuck?? Uffff. I dont know, man. 32-001
  seems to be the continuation of 32-002-2020, fix it. But the mix with 06?? Well all 06 has been removed, so.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[60]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[61]]`: Different episodes after one year??? No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[61]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[62]]`: Different episodes after years No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[62]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[63]]`: What the fuck??? No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[63]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[64]]`: No idea, no fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[64]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[65]]`: Yeah, a classic, one year is all one episode, later
  there are two episodes, just because. No fix
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[65]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[66]]`: Another classic, dancing episodes, come and see!!
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[66]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[67]]`: What in the fucking god is this?? I'm leaning to
  remove 32 from the database.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[67]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[68]]`: 32-013 seems to be 32-017
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[68]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[69]]`: Different episodes
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[69]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[70]]`: Different episodes after years
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[70]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[71]]`: 35-012 is in reality 35-012
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[71]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[72]]`: 36-032 is in reality 35-032
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[72]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[73]]`: 40-003-2012 is in reality 40-006
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[73]], data_muddy_deboscat_checks)
```

  1. `r data_muddy_episodes_centroids_intersection[[74]]`: No idea, no fix.
  
```{r}
create_polygon_plot_2(data_muddy_episodes_centroids_intersection[[74]], data_muddy_deboscat_checks)
```
...
data_muddy_deboscat_centroids <-
  ...
  #         episode_id == '06-006' & year %in% c(2012), sf::st_area(geometry), episode_area
  #       )
  #     )
  # } %>%
  # dplyr::mutate(
  #   episode_id = dplyr::if_else(
  #     episode_id == '06-018' & year %in% c(2019), glue::glue('06-043'), episode_id
  #   )
  # ) %>%
  # dplyr::mutate(
  #   episode_id = dplyr::case_when(
  #     episode_id == '06-027' & year == 2012 ~ glue::glue('06-045'),
  #     episode_id == '06-045' & year == 2012 ~ glue::glue('06-027'),
  #     TRUE ~ episode_id
  #   )
  # ) %>%
  dplyr::filter(!stringr::str_detect(episode_id, '^06-')) %>%
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '07-026' & year %in% c(2020), glue::glue('07-019'), episode_id
    )
  ) %>%
  ...
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '14-066' & year %in% c(2020), glue::glue('14-013'), episode_id
    )
  ) %>% 
  ...
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '21-031' & year %in% c(2018), glue::glue('24-031'), episode_id
    )
  ) %>% 
  ...
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '31-002-2012' & year %in% c(2012), glue::glue('31-007-2015'), episode_id
    )
  ) %>%
  dplyr::mutate(
    episode_id = dplyr::if_else(
  ...
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '32-013' & year %in% c(2019), glue::glue('32-017'), episode_id
    )
  ) %>%
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '35-012' & year %in% c(2012), glue::glue('35-013'), episode_id
    )
  ) %>%
  dplyr::mutate(
    episode_id = dplyr::if_else(
      episode_id == '36-032' & year %in% c(2012), glue::glue('35-032'), episode_id
    )
  ) %>%
  dplyr::mutate(
    episode_id = dplyr::if_else(
...
of centroids can mess with the new/old classification), cast all geometrys as multipolygons, and create the
